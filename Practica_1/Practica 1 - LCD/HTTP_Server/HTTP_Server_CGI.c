...
osStatus_t status_lcd;
MSGQUEUE_OBJ_t_lcd msg_lcd;

int Init_MsgQueue_lcd (void){
	mid_MsgQueue_lcd = osMessageQueueNew(2, sizeof(MSGQUEUE_OBJ_t_lcd), NULL); 
  ...
  do {
    ...
    if (var[0] != 0) {
      ...
      if (strcmp (var, "led0=on") == 0) {
        ...
        sprintf(msg_lcd.texto1,"%-1s", lcd_text[0]);
        osThreadFlagsSet (tid_Thlcd, 0x10);
        osMessageQueuePut(mid_MsgQueue_lcd,&msg_lcd,0U,0U);
      }
      else if (strncmp (var, "lcd2=", 5) == 0) {
        // LCD Module line 2 text
        strcpy (lcd_text[1], var+5);
        sprintf(msg_lcd.texto2,"%-1s", lcd_text[1]);
        osThreadFlagsSet (tid_Thlcd, 0x10);
        osMessageQueuePut(mid_MsgQueue_lcd,&msg_lcd,0U,0U);
      }
      else{
